Shader"ENTI/P6_Gold"
{
    Properties
    {
        ...
        [Header(Sharpness)]
        _Sharpness("Sharpness", Range(0.005,1)) = 1.0
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag(v2f i) : SV_TARGET
            {
                ...
                //2. metalness
                float metalness = saturate(max(_Sharpness, tex2D(_MetalnessMap, TRANSFORM_TEX(i.uv, _MetalnessMap))).r);
                //3. roughness
                float roughness = saturate(max(_Sharpness, tex2D(_RoughnessMap, TRANSFORM_TEX(i.uv, _RoughnessMap))).r);                
                //5. normal
                ...
                normalDirection = lerp(normalDirection, worldNormal, 0.4);
                ...
                float3 diffuseColor = i.col.rgb * ((1.0 - metalness) + 1.0 / 5);
                diffuseColor = FresnelLerp(diffuseColor, 0.05, NdotV);
                float f0 = F0(NdotL, NdotV, LdotH, roughness);
                diffuseColor *= f0;

                //2. Diffuse Indirect Light
                float3 diffuseIrradiance = UNITY_SAMPLE_TEXCUBE_LOD(unity_SpecCube0, normalDirection, UNITY_SPECCUBE_LOD_STEPS).rgb;
                ...
                float3 SpecularIrradiance = UNITY_SAMPLE_TEXCUBE_LOD(unity_SpecCube0, viewReflectDirection, roughness * UNITY_SPECCUBE_LOD_STEPS).rgb;
